...
function rowValues(story, index) {
  ...
  const replayContract = `seed=${story.seed}／${story.plannerVersion || "boss-plan-v11"}／story-action-trace-v2／deviation-suppression-v4-configurable-tables`;
  return [
...
async function buildSummarySheet(statsRows) {
  ...
  rows.push(`<row r="2">${stringCell("A2", "版本：frontend-v96／action-tree-v35／boss-plan-v11／arrange-v10／natural-story-preset-v13；24 個結果資料格各 10,000；同一 X 倍數劇本通用所有 Bet。", 2)}</row>`);
