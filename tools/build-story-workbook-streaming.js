...
function rowValues(story, index) {
  ...
  const replayContract = `seed=${story.seed}／${story.plannerVersion || "boss-plan-v10"}／story-action-trace-v1／deviation-suppression-v3-configurable-tables`;
  return [
...
async function buildSummarySheet(statsRows) {
  ...
  rows.push(`<row r="2">${stringCell("A2", "版本：frontend-v92／action-tree-v33／boss-plan-v10／arrange-v9／natural-story-preset-v11；24 個結果資料格各 10,000；同一 X 倍數劇本通用所有 Bet。", 2)}</row>`);
