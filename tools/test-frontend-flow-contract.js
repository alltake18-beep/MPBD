...
const css = fs.readFileSync(path.join(root, "boss-duel-demo.css"), "utf8");
...
assert(js.includes('findSlot("cloth")') && js.includes("legacyMaximumCloth.attachment = null") && js.includes("legacyMaximumCloth.color.a = 0"), "legacy baked treasure maximum must stay hidden beneath the dedicated Boss plaque");
...
assert(js.includes("Rules.createNaturalRound") && js.includes("1201 + tieIndex * 17") && js.includes("if (!Number.isInteger(encounter.packet.naturalStorySeed))"), "story experience must replay the natural deal/redraw seed and bypass the legacy controlled compare rewrite");
