...
(function actionTreeLab() {
  ...
  function carryChecks() {
    ...
    if (c.rewardFloorPct !== 10) issues.push("新逐利玩家 BOSS 原骰獎最低必須為原獎 10%");
    if (c.rewardCeilingMultiple !== 1000) issues.push("新逐利玩家 BOSS 原骰獎最高必須為原獎 1,000 倍");
    return { pass: issues.length === 0, issues };
  }
  ...
  function buildMechanics() {
    ...
    const playerBehaviorOptions = [
      ["SMART", "新逐利玩家（真實劇情／含理牌）"],
      ["OFFICIAL_FUNDED", "原版聰明玩家（官方策略）"],
      ["FREE_RIDE", "原版白嫖玩家"],
      ["EXTREME", "原版極端玩家"]
    ];
    ...
    $("simulationGrid").innerHTML = `
      ...
      <section class="param-group">
        ...
        <div class="field-list">
          ...
          ${simulationInput("earlyTerminationPct", "原版模型提前離開率（%）", 0.1)}
        </div>
      </section>
  ...
  function prepareLegacyReport(result) {
    ...
    result.riskFindings = [{
      ...
      evidence: "本次使用原版統計玩家模型，不抽新逐利玩家的真實故事候選。",
    ...
    result.settlementFunnel = { candidateStoriesDrawn: 0, commits: t.bosses, settlements: t.bosses, pending: 0, note: "原版玩家統計模型不使用真實故事候選" };
  ...
  function renderSimulation(result) {
    if (!result.ticketHealth) result = prepareLegacyReport(result);
    ...
    const behaviorLabels = {
      SMART: "新逐利玩家（真實劇情／含理牌）", OFFICIAL_FUNDED: "原版聰明玩家（官方策略）", FREE_RIDE: "原版白嫖玩家", EXTREME: "原版極端玩家",
    ...
    $("runInfoBody").innerHTML = [
      ...
      ["統計花費時間", `${Math.max(0, result.runInfo.reportElapsedMs)}ms`, result.config.simulation.playerBehavior === "SMART" ? "真實故事池逐局動態配籤" : "原版獨立玩家統計模型"],
      ...
      ["玩家數", count(result.config.simulation.playerCount), smartStoryModel ? "新版故事模型的獨立退幣統計尚未建立，報表留空" : `退幣另跑 ${count(result.config.simulation.cashoutPlayerCount)} 人`],
      ...
      ["遊戲故事池", result.config.simulation.playerBehavior === "SMART" ? esc(result.config.versions.storyPool) : "不適用（原版統計模型）", result.config.simulation.playerBehavior === "SMART" ? "每星贏多、贏少、輸各固定 10,000 局；本輪主模擬沿劇本操作，實際玩家偏離分布待後續策略確認" : "原版三種玩家模型使用獨立統計路徑"]
    ].map((row) => rowHtml(row, true)).join("");
    ...
    $("ticketHealthBody").innerHTML = [
      ...
      ["分類比例目標", "未設定", "新版沒有 6／55／39 或其他固定分類比例"],
  ...
  function runRuntimeSimulation() {
    ...
    $("simulationState").textContent = usesNaturalStories
      ? `正在以新逐利玩家重播真實劇情，從三個完整分類各等機率抽 1 個候選，再配成 ${config.targetCoreRtpPct}%…`
      : `正在以${config.simulation.playerBehavior === "OFFICIAL_FUNDED" ? "原版聰明" : config.simulation.playerBehavior === "FREE_RIDE" ? "原版白嫖" : "原版極端"}玩家跑獨立統計模型…`;
    $("runSimulationButton").disabled = true;
    requestAnimationFrame(() => {
      try {
        simulationResult = usesNaturalStories
          ? Core.simulateNaturalModel(config, { pool: catalogPool })
          : Core.simulateLegacy(config);
        simulationHash = currentHash();
        ...
        $("simulationState").textContent = usesNaturalStories
          ...
          : `完成：${count(simulationResult.totals.bosses)} 隻 BOSS；已套用原版玩家行為與退幣統計。`;
      } catch (error) {
  ...
  function statisticsExportValue() {
    ...
    return {
      ...
      phase: storyPoolCells.length === 24 ? "BUILT_IN_STORY_POOL_READY" : "LEGACY_PLAYER_MODEL",
