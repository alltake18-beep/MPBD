...
目前執行期抑制為 `deviation-suppression-v3-configurable-tables`：開卡時只有金幣卡公開數值，其餘傷害卡只公開種類；比牌時若沒有抑制就使用正常表後端隱藏值，若有抑制則完全不參照原值，暴擊、固傷與共用牌型傷害倍率各自改抽專用表。預設為暴擊 x1／x2／x3／x4／x5＝69%／25%／3%／2%／1%；固傷 +3／+4／+5／+6＝70%／25%／3%／2%；牌型傷害 x1／x2／x3＝80%／19%／1%。完整結果、百分比、啟用狀態與簽章必須隨 Boss 鎖定並可重播。
