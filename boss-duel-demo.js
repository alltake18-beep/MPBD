...
(function runBossDuelDemo() {
  ...
  const PLAYER_STATE_KEY = "boss-duel:dice-first:demo-player";
  ...
  const MAGIC_ART_PATHS = Object.freeze({
    ...
    fullHouseBoost: "assets/mobile/magic-cards/full-house-base.png",
  ...
  async function showTreasureSkin(encounterId, star, maximum, maximumRevealed) {
    ...
    try {
      ...
      const legacyMaximumCloth = activeTreasureSpine.skeleton.findSlot("cloth");
      if (legacyMaximumCloth) legacyMaximumCloth.attachment = null;
      activeTreasureSpine.state.setAnimation(0, "appear", false);
      activeTreasureSpine.state.addAnimation(0, "idle", true, 0);
      activeTreasureSpine.update(0);
      if (legacyMaximumCloth) {
        legacyMaximumCloth.attachment = null;
        legacyMaximumCloth.color.a = 0;
      }
  ...
  function loadConfig() {
    try {
      return Core.sanitizeConfig(JSON.parse(localStorage.getItem(Core.STORAGE_KEY) || "{}"));
    } catch (_error) {
      return Core.sanitizeConfig(Core.DEFAULT_CONFIG);
    }
  }
  ...
  function runtimeStoryRng(star) {
    ...
    for (let index = 0; index < String(playerState.id).length; index += 1) {
      playerSeed = Core.hash32(playerSeed, String(playerState.id).charCodeAt(index), index + 1);
    }
    const storyIndex = playerState.epoch * runtimeConfig.cycleSize + playerState.index;
    return Core.mulberry32(Core.hash32(playerSeed, storyIndex, 6200 + star));
  }
  ...
  function activeBetLedger() {
    return Core.betLedger(playerState, activeBet);
  }
  ...
  function ensureCycle() {
    if (cycle && cycle.epoch === playerState.epoch && cycle.playerId === playerState.id && cycle.config.revision === runtimeConfig.revision) return;
    cycle = Core.buildPersonalCycle(runtimeConfig, playerState.id, playerState.epoch);
    if (playerState.index >= cycle.packets.length) {
      playerState.epoch += 1;
      playerState.index = 0;
      cycle = Core.buildPersonalCycle(runtimeConfig, playerState.id, playerState.epoch);
      savePlayerState();
  ...
  function spawnBoss(avoidStar = 0) {
    applyPendingConfig();
    ensureCycle();
    if (avoidStar && cycle.packets[playerState.index]?.star === avoidStar) {
      const replacementIndex = cycle.packets.findIndex((candidate, index) => index > playerState.index && candidate.star !== avoidStar);
      if (replacementIndex > playerState.index) {
        [cycle.packets[playerState.index], cycle.packets[replacementIndex]] = [cycle.packets[replacementIndex], cycle.packets[playerState.index]];
      }
    }
    ...
    const packet = story ? {
      ...
      storyArchetype: activeStoryExperience.archetype,
  ...
  function fight() {
    ...
    if (!Number.isInteger(encounter.packet.naturalStorySeed)) {
      const targetReady = encounter.packet.win && encounter.round >= encounter.packet.targetRound && encounter.draws >= encounter.packet.requiredDraws;
      Rules.prepareCompare(state, !targetReady, encounter.hpLeft);
    }
    // 比牌以最終最佳五張為準；原版直接在牌桌完成理牌與 BOSS 攤牌，不插入全屏 VS 卡。
  ...
  function beginBossDefeat() {
    ...
    playSfx("win");
    render();
    // 原版由 BOSS 自身死亡 Spine 與寶箱升級承接，不插入自創的全屏 DEFEATED 字卡。
    els.bossDefeatFx.hidden = true;
  ...
  function createCurrentRound(tieRedeal = false) {
    ...
    if (Number.isInteger(encounter.packet.naturalStorySeed)) {
      ...
      const roundSeed = Core.hash32(encounter.packet.naturalStorySeed, encounter.round, 1201 + tieIndex * 17);
      ...
      encounter.presentation = Rules.createRound(encounter.packet, encounter.round, encounter.hpLeft, encounter.tieRedeals || 0);
    }
    const storyStep = encounter.packet.storyRecord?.path?.find((step) =>
      step.round === encounter.round && step.tieIndex === (encounter.tieRedeals || 0)
    );
  ...
  function receiveConfig(data) {
    const next = Core.sanitizeConfig(data?.config || data || {});
    if (next.revision <= runtimeConfig.revision && (!pendingConfig || next.revision <= pendingConfig.revision)) return;
    pendingConfig = next;
