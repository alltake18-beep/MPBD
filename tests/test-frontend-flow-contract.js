...
const finishRoundBody = sourceBetween(js, "function finishRound(", "function hideBossSpeech(");
...
const dealRoundBody = sourceBetween(js, "function dealRound(", "function beginEntryStarReveal(");
const continueRoundBody = sourceBetween(js, "function continueRound(", "function beginBossDefeat(");
...
assert(js.includes('document.documentElement.dataset.rewardDiceError = ""') && js.includes("void mountRewardDiceSpines()") && js.includes("REWARD_DIE_FLIP_SECONDS = 0.48") && css.includes(".die.rolling"), "reward dice must mount the repaired Spine atlas and visibly finish its shortened flip before revealing each locked face");
...
assert(js.includes("const optionKeys = Object.keys(MAGIC_ART_PATHS)") && js.includes("const selectedOrder = new Map(cards.map((card, index) => [card.key, index]))") && js.includes('data-magic-option="${key}"') && css.includes(".magic-draw-fan > span:nth-child(10)") && css.includes(".magic-draw-fan > span.selected") && css.includes("@keyframes magic-draw-picked"), "magic draw must present all ten distinct magic options and visibly pick the two locked results without redrawing them");
...
const attackSpineBody = sourceBetween(js, "async function playAttackSpine(", "function measureBossSafeBounds(");
...
const attackBody = sourceBetween(js, "async function beginAttackPlayback(", "async function showCombatResolution(");
...
assert(attackBody.indexOf('phase = "attack"') < attackBody.indexOf('phase = "damage"') && attackBody.indexOf('phase = "damage"') < attackBody.indexOf("encounter.hpLeft =") && attackBody.indexOf("encounter.hpLeft =") < attackBody.indexOf("setMessage(`-${result.damage} HP`") && attackBody.indexOf("encounter.hpLeft =") < attackBody.indexOf('phase = "post-hit"') && attackBody.indexOf('phase = "post-hit"') < attackBody.indexOf("finishRound("), "attack, damage, visible HP, idle, and CONTINUE must remain strictly ordered; the HUD must reflect damage before the damage beat renders");
...
const rewardRevealBody = sourceBetween(js, "function revealPrizeDie(index)", "function finishPrizeTotal(");
...
assert(html.includes("src/core/boss-duel-poker-arrangement-core.js?v=frontend-v102") && html.includes("src/core/boss-duel-rules.js?v=frontend-v102") && html.includes("src/core/boss-duel-natural-story-core.js?v=frontend-v102") && html.includes("src/game/boss-duel-demo.js?v=frontend-v102") && html.includes("src/game/boss-duel-demo.css?v=frontend-v102"), "Demo code, shared arrangement, and live story assets must share the v102 cache key");
...
assert(html.includes("src/core/boss-duel-story-planner.js?v=boss-plan-v11") && html.includes("data/story/boss-duel-story-preset-v1.js?v=story-catalog-v15"), "Demo must load the planner and current validated 240,000-story seed preset");
...
assert(js.includes("NaturalCore.commitStoryToBuckets") && js.includes("plannedSpendCredits: encounter.packet.storyCommit.selectedStory.spendX * activeBet") && js.includes("storyBudgetCredits: encounter.packet.storyCommit.selectedStory.payoutX * activeBet"), "starting a selected story must post its planned payout and retain its planned total spend");
assert((js.match(/spend\(runtimeConfig\.entryCostX, \{ entrySpend: true \}\)/g) || []).length >= 2 && js.includes("encounter.storySpendCredits += amount") && !js.includes("encounter.storyTargetAccrualCredits = NaturalCore.roundMoney(encounter.storyTargetAccrualCredits + targetAccrualCredits)"), "entry, continue, and redraw costs must accumulate into actual story spend without live double-posting");
assert(js.includes("storySpendDeltaTargetAccrualCredits") && js.includes("plannedSpendCredits: encounter.storyPlannedSpendCredits"), "story settlement must apply actual minus planned spend at the locked RTP");
...
assert(rerollBody.includes('spend(runtimeConfig.entryCostX, { storySpend: false })') && js.includes("entryCostX: 1"), "REROLL BOSS must charge the player current Bet × 1");
assert(rerollBody.indexOf('spend(runtimeConfig.entryCostX, { storySpend: false })') < rerollBody.indexOf("settleStoryPool(0, false)"), "an unaffordable Boss reroll must not settle or abandon the current story");
...
assert(toolHtml.includes('href="%E9%81%8A%E6%88%B2Demo.html?v=frontend-v102"'), "probability tool must keep a direct link to the current frontend Demo");
