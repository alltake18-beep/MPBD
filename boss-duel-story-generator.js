 ...
 * - dice-first-core.js：固定亂數與 seed 衍生
 * - boss-duel-rules.js：rules-v10
 * - boss-duel-story-planner.js：boss-plan-v10
 * - boss-duel-natural-story-core.js：模擬、分類、摘要與重播
 *
 * CLI：
 *   node boss-duel-story-generator.js --output <目錄> --release <版本名稱>
 *
 * 程式介面：
 *   const Generator = require("./boss-duel-story-generator.js");
...
const StoryPlanner = require("./boss-duel-story-planner.js");
const NaturalCore = require("./boss-duel-natural-story-core.js");
const ActionTreeCore = require("./boss-duel-action-tree-core.js");

const SERVICE_VERSION = "boss-duel-story-generator-v1";
const GENERATOR_REVISION = "boss-plan-v10-arrange-v9-action-trace-v1-suppression-v1-quota10000-production-v1";
const PRESET_VERSION = "natural-story-preset-v10";
const SUMMARY_PRESET_VERSION = "natural-story-summary-preset-v5";
...
const FORMAL_TICKET_GROUPS_PER_STAR = 256;
...
const EXPECTED_SUPPRESSION_VERSION = "deviation-suppression-v1";
...
function startWorker(payload) {
  return new Promise((resolve, reject) => {
    ...
    worker.once("exit", (code) => {
      if (!settled && code !== 0) reject(new StoryGenerationError("WORKER_FAILED", `worker exit ${code}`));
    });
  });
}
...
function attachCompatibilityTicketGroups(profile, preset, summaryPreset, options = {}) {
  const groupTarget = finiteInteger(
    options.groupTarget ?? (profile.formal ? FORMAL_TICKET_GROUPS_PER_STAR : 0),
    profile.formal ? FORMAL_TICKET_GROUPS_PER_STAR : 0,
    0,
    100000
  );
  if (groupTarget === 0) return preset;
  const pool = NaturalCore.buildNaturalStoryPoolFromPreset(profile.config, {
    ...preset,
    naturalSummaries: summaryPreset.naturalSummaries
  }, { useCache: false });
  assert(pool?.fromPreset, "INVALID_STORY_RESULT", "無法從新劇本索引建立相容候選群組");

  preset.ticketGroupVersion = "score-ticket-groups-v3";
  preset.ticketGroupConfig = {
    targetRtpPct: profile.config.targetRtpPct,
    ticketBasis: profile.config.ticketBasis,
    ticketPreferencePct: clone(profile.config.ticketPreferencePct)
  };
  preset.ticketGroups = {};
  preset.ticketGroupDiagnostics = [];
  for (let star = 1; star <= 8; star += 1) {
    const rng = DiceCore.mulberry32(DiceCore.hash32(profile.config.poolSeed, star, 23001));
    const groups = [];
    const seen = new Set();
    let attempts = 0;
    while (groups.length < groupTarget && attempts < groupTarget * 20) {
      const commit = NaturalCore.drawStoryCommit(pool, star, profile.config.targetRtpPct, rng, {
        ticketPreferencePct: profile.config.ticketPreferencePct,
        ticketBasis: profile.config.ticketBasis,
        ticketSearchAttempts: profile.config.ticketSearchAttempts,
        ticketCandidateTournamentSize: profile.config.ticketCandidateTournamentSize,
        ticketEarlyExitDeviationPp: profile.config.ticketEarlyExitDeviationPp
      });
    ...
    assert(groups.length === groupTarget, "QUOTA_NOT_MET", `${star} 星相容候選群組不足`, { star, actual: groups.length, expected: groupTarget, attempts });
...
function validatePreset(profileInput, preset, summaryPreset, options = {}) {
  ...
  if (profile.formal) {
    assert(totalStories === FORMAL_TOTAL_STORIES, "QUOTA_NOT_MET", "正式故事總數必須是 240,000", { actual: totalStories });
    assert(preset.ticketGroupVersion === "score-ticket-groups-v3", "QUOTA_NOT_MET", "正式索引缺少現行相容候選群組版本");
    assert(preset.ticketGroupConfig?.targetRtpPct === profile.config.targetRtpPct, "SIGNATURE_MISMATCH", "相容候選群組 RTP 設定不同");
    assert(preset.ticketGroupConfig?.ticketBasis === profile.config.ticketBasis, "SIGNATURE_MISMATCH", "相容候選群組票基數不同");
    assert(stableJson(preset.ticketGroupConfig?.ticketPreferencePct) === stableJson(profile.config.ticketPreferencePct), "SIGNATURE_MISMATCH", "相容候選群組偏好設定不同");
    for (let star = 1; star <= 8; star += 1) {
      assert(Array.isArray(preset.ticketGroups?.[star]) && preset.ticketGroups[star].length === FORMAL_TICKET_GROUPS_PER_STAR, "QUOTA_NOT_MET", `${star} 星相容候選群組不是 ${FORMAL_TICKET_GROUPS_PER_STAR} 組`);
      const classSeedSets = CLASS_KEYS.map((classKey) => new Set(preset.natural[star][classKey].map((seed) => Number(seed) >>> 0)));
      const groupKeys = new Set();
      for (const group of preset.ticketGroups[star]) {
        assert(Array.isArray(group) && group.length === CLASS_KEYS.length, "INVALID_STORY_RESULT", `${star} 星相容候選群組必須含三個 seed`);
        const groupKey = group.map((seed) => Number(seed) >>> 0).join(":");
        assert(!groupKeys.has(groupKey), "INVALID_STORY_RESULT", `${star} 星相容候選群組不得重複`, { star, groupKey });
        groupKeys.add(groupKey);
        group.forEach((seed, index) => {
          assert(classSeedSets[index].has(Number(seed) >>> 0), "INVALID_STORY_RESULT", `${star} 星相容候選群組的 seed 不在對應結果分類`, { star, classKey: CLASS_KEYS[index], seed });
        });
      }
    }
  }
...
async function buildRelease(options = {}) {
  ...
  attachCompatibilityTicketGroups(profile, preset, summaryPreset);
  ...
  const presetFile = path.join(stagingRoot, "boss-duel-story-preset-v1.js");
  const summaryFile = path.join(stagingRoot, "boss-duel-story-summary-preset-v1.js");
  const diagnosticsFile = path.join(stagingRoot, "natural-story-diagnostics.json");
  ...
  const manifest = {
    ...
    compatibilityTicketGroupsPerStar: FORMAL_TICKET_GROUPS_PER_STAR,
...
const publicApi = {
  ...
  buildPreset,
  attachCompatibilityTicketGroups,
  validatePreset,
  validateReplayBatch,
  validateAllReplays,
  buildRelease,
