...
> 可執行程式：`boss-duel-story-generator.js`
>
...
本文件只說明接法。下列邏輯已實作在 `boss-duel-story-generator.js`：
...
```js
const StoryGenerator = require("./boss-duel-story-generator.js");
```
...
dice-first-core.js
boss-duel-rules.js
boss-duel-story-planner.js
boss-duel-natural-story-core.js
boss-duel-action-tree-core.js
```
...
```bash
node boss-duel-story-generator.js \
...
`materializeStoryForBet()` 直接委派給 `boss-duel-natural-story-core.js` 的唯一 `story-bet-scaling-v1` 實作，不修改原劇本，也不重新發牌。若呼叫端需要限制投注清單，可選擇傳入：
...
```bash
node tools/test-story-generator-service.js
```
